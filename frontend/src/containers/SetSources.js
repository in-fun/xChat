...
export default function SetSources({
  ...
  const dataIcons = {
    ...
    docx: <DocIcon className="w-5 h-5 mr-3" />,
  ...
  const handleAddDataSource = async (e) => {
    ...
    const addDataSourceEntry = {
      sender: "B",
      message: `Adding the following ${dataTypes[sourceType]}: ${sourceValue}`,
    };
    ...
    if (response.ok) {
      const successEntry = {
        sender: "B",
        message: `Successfully added ${dataTypes[sourceType]}!`,
      };
      setChats((prevChats) => [...prevChats, successEntry]);
    } else {
      const errorEntry = {
        sender: "B",
        message: `Failed to add ${dataTypes[sourceType]}. Please try again.`,
      };
